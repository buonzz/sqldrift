...
class SqlDrift extends Command {
  ...
  async run() {
    ...
    try {
      ...
      const historyFilePath = path.join(sqldriftDir, `${sqlFileName}-history.json`)
...
db = "sqldrift"`
  ...
  async loadConfig(configPath) {
    ...
    if (!config.default) {
      this.error('Configuration file must contain a [default] section')
    }
  ...
  async confirmExecution(statements, dbName) {
    ...
    return new Promise((resolve) => {
      rl.question('Do you want to proceed with executing these statements? (y/N): ', (answer) => {
        rl.close()
