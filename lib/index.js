  ...
  async run() {
    ...
    try {
      ...
      if (!openRouterConfig || !openRouterConfig.api_key) {
        ...
        this.log(chalk.gray('model = "anthropic/claude-3.5-sonnet"'))
      ...
      this.log(chalk.cyan('LLM service initialized with OpenRouter'))
...
model = "anthropic/claude-3.5-sonnet"`
  ...
  async parseSqlFile(sqlFilePath, llmService) {
    ...
    this.log(chalk.cyan('Using LLM to parse SQL file...'))
    
    try {
      const statements = await llmService.splitSqlFile(content)
  ...
  async executeStatements(statements, config, history, historyFilePath, llmService) {
    ...
    try {
      ...
      for (let i = 0; i < statements.length; i++) {
        ...
        this.log(`${progress} ${chalk.cyan('Validating:')} ${chalk.white(truncatedSql)}${suffix}`)
        
        try {
          const validation = await llmService.validateSqlStatement(statement.sql)
