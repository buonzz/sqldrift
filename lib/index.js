...
class SqlDrift extends Command {
  ...
  async run() {
    ...
    if (flags.help || !args.sqlfile) {
      this.log(chalk.cyan('SqlDrift - SQL Migration Tool with LLM Integration\n'))
    ...
    try {
      ...
      // Load Ollama configuration (optional)
      const ollamaConfig = await this.loadOllamaConfig(flags.config)
      
      // Initialize LLM service with Ollama
      const model = ollamaConfig?.model || 'llama3.2:1b'
      const llmService = new LLMService(null, model)
      
      // Test the connection with progress indicator
      this.log(chalk.cyan('🚀 Initializing LLM service with Ollama...'))
      try {
        const testResult = await llmService.testConnection(this.log.bind(this))
        if (testResult.success) {
          this.log(chalk.green('✅ Ollama service ready'))
        } else {
          this.log(chalk.yellow(`⚠️  Ollama service warning: ${testResult.message}`))
          this.log(chalk.yellow('Make sure Ollama is running: ollama serve'))
          this.log(chalk.yellow(`Make sure model is installed: ollama pull ${model}`))
        }
      } catch (testError) {
        this.log(chalk.red(`❌ Could not connect to Ollama: ${testError.message}`))
        this.log(chalk.yellow('\nTo use SqlDrift with Ollama:'))
        this.log(chalk.cyan('1. Install Ollama: https://ollama.ai/'))
        this.log(chalk.cyan('2. Start Ollama: ollama serve'))
        this.log(chalk.cyan(`3. Install a model: ollama pull ${model}`))
        this.log(chalk.cyan('4. Optionally configure model in ~/.sqldrift/default.conf:'))
        this.log(chalk.gray('\n[ollama]'))
        this.log(chalk.gray('model = "llama3.2:3b"'))
        process.exit(1)
      }
...
model = "llama3.2:1b"`
  ...
  async loadOllamaConfig(configPath) {
    try {
      if (!await fs.pathExists(configPath)) {
        return null
      }
      ...
      this.log(chalk.yellow(`Warning: Could not load Ollama configuration: ${error.message}`))
      return null
    }
  }
