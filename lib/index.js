const { Command, Flags, Args } = require('@oclif/core')
const fs = require('fs-extra')
const path = require('path')
const os = require('os')
const mysql = require('mysql2/promise')
const readline = require('readline')
const chalk = require('chalk')
const cliProgress = require('cli-progress')
const SqlParserService = require('./services/sql-parser-service')

class SqlDrift extends Command {
  static description = 'Run SQL migrations against MySQL database with tracking'

  // Helper method to get formatted timestamp
  getTimestamp() {
    return new Date().toLocaleTimeString('en-US', {
      hour12: false,
      hour: '2-digit',
      minute: '2-digit',
      second: '2-digit'
    })
  }

  // Override log method to include timestamp
  log(message) {
    const timestamp = chalk.gray(`[${this.getTimestamp()}]`)
    super.log(`${timestamp} ${message}`)
  }

  static examples = [
    '<%= config.bin %> path/to/migrations.sql',
    '<%= config.bin %> path/to/migrations.sql --config=custom.cnf',
    '<%= config.bin %> path/to/migrations.sql --environment=production',
    '<%= config.bin %> path/to/migrations.sql --config=custom.cnf --environment=staging',
  ]

  static flags = {
    config: Flags.string({
      char: 'c',
      description: 'path to configuration file',
      default: path.join(os.homedir(), '.sqldrift', 'default.conf')
    }),
    environment: Flags.string({
      char: 'e',
      description: 'environment to use from configuration file',
      default: 'default'
    }),
    'record-history': Flags.boolean({
      description: 'only update history file without executing SQL statements against the database',
      default: false
    }),
    'clear-history': Flags.boolean({
      description: 'delete history file(s) for the specified environment (without sqlfile: clears ALL history files)',
      default: false
    }),
    'view-history': Flags.boolean({
      description: 'display the execution history in tabular format with pagination',
      default: false
    }),
    help: Flags.boolean({
      char: 'h',
      description: 'show help',
      default: false
    }),
  }

  static args = {
    sqlfile: Args.string({
      description: 'path to SQL file to execute',
      required: false
    })
  }

  async run() {
    const { args, flags } = await this.parse(SqlDrift)
    
    // Handle clear-history mode without SQL file (clear all history files)
    if (flags['clear-history'] && !args.sqlfile) {
      // Ensure .sqldrift directory exists
      const sqldriftDir = path.join(os.homedir(), '.sqldrift')
      await fs.ensureDir(sqldriftDir)
      
      await this.clearAllHistory(sqldriftDir, flags.environment)
      return
    }
    
    // Handle view-history mode
    if (flags['view-history']) {
      if (!args.sqlfile) {
        this.error('SQL file path is required when using --view-history flag')
      }
      
      // Resolve SQL file path
      const sqlFilePath = path.resolve(args.sqlfile)
      const sqlFileName = path.basename(sqlFilePath)
      const pathHash = this.generateHash(sqlFilePath)
      
      // Ensure .sqldrift directory exists
      const sqldriftDir = path.join(os.homedir(), '.sqldrift')
      await fs.ensureDir(sqldriftDir)
      
      const historyFilePath = path.join(sqldriftDir, `${flags.environment}-${sqlFileName}-${pathHash}-history.json`)
      
      await this.viewHistory(historyFilePath, flags.environment, sqlFileName)
      return
    }
    
    // If help flag is provided or no SQL file is provided, show usage information
    if (flags.help || !args.sqlfile) {
      this.log(chalk.cyan('SqlDrift - SQL Migration Tool\n'))
      this.log(chalk.white('Usage:'))
      this.log(chalk.gray('  sqldrift <sqlfile> [options]'))
      this.log(chalk.gray('  sqldrift --clear-history [options]\n'))
      this.log(chalk.white('Arguments:'))
      this.log(chalk.gray('  sqlfile  Path to SQL file to execute\n'))
      this.log(chalk.white('Options:'))
      this.log(chalk.gray('  -c, --config <file>        Path to configuration file'))
      this.log(chalk.gray('                             (default: ~/.sqldrift/default.conf)'))
      this.log(chalk.gray('  -e, --environment <env>    Environment to use from config'))
      this.log(chalk.gray('                             (default: default)'))
      this.log(chalk.gray('  --record-history           Only update history without executing SQL'))
      this.log(chalk.gray('  --clear-history            Delete history file(s) for environment'))
      this.log(chalk.gray('                             (without sqlfile: clears ALL history files)'))
      this.log(chalk.gray('  --view-history             Display execution history in tabular format'))
      this.log(chalk.gray('  -h, --help                 Show help\n'))
      this.log(chalk.white('Examples:'))
      this.log(chalk.gray('  sqldrift migrations.sql'))
      this.log(chalk.gray('  sqldrift migrations.sql --config=custom.cnf'))
      this.log(chalk.gray('  sqldrift migrations.sql --environment=production'))
      this.log(chalk.gray('  sqldrift migrations.sql --record-history'))
      this.log(chalk.gray('  sqldrift migrations.sql --clear-history'))
      this.log(chalk.gray('  sqldrift --clear-history --environment=development'))
      this.log(chalk.gray('  sqldrift migrations.sql --view-history --environment=development\n'))
      this.log(chalk.yellow('For more information, visit: https://github.com/buonzz/sqldrift'))
      return
    }
    
    try {
      // Resolve SQL file path
      const sqlFilePath = path.resolve(args.sqlfile)
      
      if (!await fs.pathExists(sqlFilePath)) {
        this.error(`SQL file not found: ${sqlFilePath}`)
      }

      // Ensure .sqldrift directory exists
      const sqldriftDir = path.join(os.homedir(), '.sqldrift')
      await fs.ensureDir(sqldriftDir)

      // Create default config if it doesn't exist
      await this.ensureDefaultConfig(sqldriftDir)

      // Load configuration
      const config = await this.loadConfig(flags.config, flags.environment)
      
      // Initialize SQL parser service
      const sqlParserService = new SqlParserService()
      
      // Test the parser functionality
      //this.log(chalk.cyan('🚀 Initializing SQL parser service...'))
      try {
        const testResult = await sqlParserService.testConnection(this.log.bind(this))
        if (testResult.success) {
          //this.log(chalk.green('✅ SQL parser service ready'))
        } else {
          this.log(chalk.red(`❌ SQL parser service error: ${testResult.message}`))
          process.exit(1)
        }
      } catch (testError) {
        this.log(chalk.red(`❌ Could not initialize SQL parser: ${testError.message}`))
        process.exit(1)
      }

      // Get history file path with unique identifier based on full path
      const sqlFileName = path.basename(sqlFilePath)
      // Create a unique identifier from the full path to avoid conflicts
      const pathHash = this.generateHash(sqlFilePath)
      const historyFilePath = path.join(sqldriftDir, `${flags.environment}-${sqlFileName}-${pathHash}-history.json`)

      // Handle clear-history mode
      if (flags['clear-history']) {
        await this.clearHistory(historyFilePath, flags.environment, sqlFileName)
        return
      }

      // Load or create history
      const history = await this.loadHistory(historyFilePath)

      // Parse SQL file using SQL parser
      const sqlStatements = await this.parseSqlFile(sqlFilePath, sqlParserService)

      // Find new statements
      const newStatements = this.findNewStatements(sqlStatements, history)

      if (newStatements.length === 0) {
        this.log(chalk.yellow('No new SQL statements to execute.'))
        return
      }

      // Handle record-history mode
      if (flags['record-history']) {
        await this.recordHistoryOnly(newStatements, history, historyFilePath, sqlParserService)
        return
      }

      // Prompt user for confirmation
      const confirmed = await this.confirmExecution(newStatements, config.db)
      
      if (!confirmed) {
        this.log(chalk.yellow('Execution cancelled.'))
        return
      }

      // Execute statements with SQL validation
      await this.executeStatements(newStatements, config, history, historyFilePath, sqlParserService)

    } catch (error) {
      // Handle different types of errors with user-friendly messages
      if (error.message.includes('SQL parser')) {
        this.log(chalk.red('\n❌ SQL Parser Error'))
        this.log(chalk.white(`${error.message}`))
        this.log(chalk.yellow('\nPlease check your SQL syntax and try again.\n'))
      } else {
        this.log(chalk.red('\n❌ Error'))
        this.log(chalk.white(`${error.message}\n`))
      }
      process.exit(1)
    }
  }

  async ensureDefaultConfig(sqldriftDir) {
    const defaultConfigPath = path.join(sqldriftDir, 'default.conf')
    
    if (!await fs.pathExists(defaultConfigPath)) {
      const defaultConfig = `[default]
user = "root"
password = "root"
host = "127.0.0.1"
port = "3306"
db = "sqldrift"

[development]
user = "root"
password = "root"
host = "127.0.0.1"
port = "3306"
db = "sqldrift_dev"

[staging]
user = "root"
password = "root"
host = "127.0.0.1"
port = "3306"
db = "sqldrift_stage"

[production]
user = "root"
password = "root"
host = "127.0.0.1"
port = "3306"
db = "sqldrift_prod"`

      await fs.writeFile(defaultConfigPath, defaultConfig)
      this.log(chalk.green(`Created default configuration at: ${defaultConfigPath}`))
    }
  }

  async loadConfig(configPath, environment = 'default') {
    if (!await fs.pathExists(configPath)) {
      this.error(`Configuration file not found: ${configPath}`)
    }

    const configContent = await fs.readFile(configPath, 'utf8')
    const config = this.parseIniConfig(configContent)
    
    if (!config[environment]) {
      this.error(`Configuration file must contain a [${environment}] section`)
    }

    return config[environment]
  }


  parseIniConfig(content) {
    const config = {}
    let currentSection = null

    const lines = content.split('\n')
    for (const line of lines) {
      const trimmed = line.trim()
      
      if (trimmed === '' || trimmed.startsWith('#') || trimmed.startsWith(';')) {
        continue
      }

      if (trimmed.startsWith('[') && trimmed.endsWith(']')) {
        currentSection = trimmed.slice(1, -1)
        config[currentSection] = {}
        continue
      }

      if (currentSection && trimmed.includes('=')) {
        const [key, ...valueParts] = trimmed.split('=')
        const value = valueParts.join('=').trim().replace(/^["']|["']$/g, '')
        config[currentSection][key.trim()] = value
      }
    }

    return config
  }

  async loadHistory(historyFilePath) {
    if (await fs.pathExists(historyFilePath)) {
      return await fs.readJson(historyFilePath)
    }
    return { executed: [] }
  }

  async parseSqlFile(sqlFilePath, sqlParserService) {
    const content = await fs.readFile(sqlFilePath, 'utf8')
    
    try {
      this.log(chalk.cyan('📡 Checking SQL file ' + sqlFilePath));
      const statements = await sqlParserService.splitSqlFile(content, this.log.bind(this))
      
      return statements.map((stmt, index) => ({
        id: index + 1,
        sql: stmt.trim(),
        hash: this.generateHash(stmt.trim())
      }))
    } catch (error) {
      this.log(chalk.red('\n❌ SQL parsing failed'))
      this.log(chalk.white(`${error.message}`))
      this.log(chalk.yellow('\nPlease check your SQL syntax and try again.\n'))
      process.exit(1)
    }
  }

  generateHash(content) {
    // Simple hash function for tracking statements
    let hash = 0
    for (let i = 0; i < content.length; i++) {
      const char = content.charCodeAt(i)
      hash = ((hash << 5) - hash) + char
      hash = hash & hash // Convert to 32-bit integer
    }
    return hash.toString()
  }

  findNewStatements(statements, history) {
    const executedHashes = new Set(history.executed.map(item => item.hash))
    return statements.filter(stmt => !executedHashes.has(stmt.hash))
  }

  async confirmExecution(statements, dbName) {
    this.log(chalk.cyan(`\nFound ${statements.length} new SQL statement(s) to execute against database: ${chalk.bold(dbName)}`))
    
    statements.forEach((stmt, index) => {
      const truncatedSql = stmt.sql.substring(0, 100)
      const suffix = stmt.sql.length > 100 ? chalk.dim('...') : ''
      this.log(chalk.blue(`${index + 1}.`) + ` ${chalk.white(truncatedSql)}${suffix}`)
    })

    // Use readline for confirmation
    const rl = readline.createInterface({
      input: process.stdin,
      output: process.stdout
    })

    return new Promise((resolve) => {
      rl.question(chalk.yellow('Do you want to proceed with executing these statements? (y/N): '), (answer) => {
        rl.close()
        resolve(answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes')
      })
    })
  }

  async executeStatements(statements, config, history, historyFilePath, sqlParserService) {
    const connection = await mysql.createConnection({
      host: config.host,
      port: parseInt(config.port),
      user: config.user,
      password: config.password,
      database: config.db
    })

    try {
      await connection.beginTransaction()

      this.log(chalk.cyan(`\nStarting execution of ${statements.length} statement(s)...\n`))

      for (let i = 0; i < statements.length; i++) {
        const statement = statements[i]
        const truncatedSql = statement.sql.substring(0, 100)
        const suffix = statement.sql.length > 100 ? chalk.dim('...') : ''
        
        // Show progress as part of the log message
        const progress = chalk.blue(`[${i + 1}/${statements.length}]`)
        
        // Validate statement with SQL parser
        try {
          const validation = await sqlParserService.validateSqlStatement(statement.sql, (msg) => {
            this.log(`${progress} ${msg}`)
          })
          
          if (!validation.isValid) {
            await connection.rollback()
            
            this.log(chalk.red(`\n❌ SQL Statement validation failed:`))
            this.log(chalk.white(`Statement: ${statement.sql}`))
            this.log(chalk.red(`Severity: ${validation.severity || 'unknown'}`))
            
            if (validation.issues && validation.issues.length > 0) {
              this.log(chalk.red('\nIssues found:'))
              validation.issues.forEach((issue, idx) => {
                this.log(chalk.red(`  ${idx + 1}. ${issue}`))
              })
            }
            
            if (validation.recommendations && validation.recommendations.length > 0) {
              this.log(chalk.yellow('\nRecommendations:'))
              validation.recommendations.forEach((rec, idx) => {
                this.log(chalk.yellow(`  ${idx + 1}. ${rec}`))
              })
            }
            
            this.error('Execution halted due to SQL validation failure. Please fix the issues and try again.')
          }
          
          // Log any recommendations even for valid statements
          if (validation.recommendations && validation.recommendations.length > 0) {
            this.log(chalk.yellow(`${progress} ${chalk.yellow('Recommendations:')} ${validation.recommendations.join(', ')}`))
          }
          
        } catch (validationError) {
          await connection.rollback()
          this.log(chalk.red('\n❌ SQL validation failed'))
          this.log(chalk.white(`${validationError.message}`))
          this.log(chalk.yellow('\nPlease check your SQL syntax and try again.\n'))
          process.exit(1)
        }
        
        this.log(`${progress} ${chalk.magenta('Executing:')} ${chalk.white(truncatedSql)}${suffix}`)
        
        await connection.execute(statement.sql)
        
        // Add to history
        history.executed.push({
          id: statement.id,
          hash: statement.hash,
          sql: statement.sql,
          executedAt: new Date().toISOString()
        })
      }

      await connection.commit()
      
      // Save updated history
      await fs.writeJson(historyFilePath, history, { spaces: 2 })
      
      this.log(chalk.green(`\nSuccessfully executed ${statements.length} statement(s)`))
      this.log(chalk.gray(`History updated: ${historyFilePath}`))

    } catch (error) {
      await connection.rollback()
      this.error(`Transaction failed and was rolled back: ${error.message}`)
    } finally {
      await connection.end()
    }
  }

  async recordHistoryOnly(statements, history, historyFilePath, sqlParserService) {
    this.log(chalk.cyan(`\nValidating and recording ${statements.length} new SQL statement(s) to history without executing...\n`))
    
    const validStatements = []
    let skippedCount = 0

    // Validate each statement before recording
    for (let i = 0; i < statements.length; i++) {
      const statement = statements[i]
      const truncatedSql = statement.sql.substring(0, 100)
      const suffix = statement.sql.length > 100 ? chalk.dim('...') : ''
      
      // Show progress as part of the log message
      const progress = chalk.blue(`[${i + 1}/${statements.length}]`)
      
      // Validate statement with SQL parser
      try {
        const validation = await sqlParserService.validateSqlStatement(statement.sql, (msg) => {
          this.log(`${progress} ${msg}`)
        })
        
        if (!validation.isValid) {
          this.log(chalk.red(`${progress} ❌ Skipping invalid SQL statement:`))
          this.log(chalk.white(`  Statement: ${truncatedSql}${suffix}`))
          this.log(chalk.red(`  Severity: ${validation.severity || 'unknown'}`))
          
          if (validation.issues && validation.issues.length > 0) {
            this.log(chalk.red('  Issues found:'))
            validation.issues.forEach((issue, idx) => {
              this.log(chalk.red(`    ${idx + 1}. ${issue}`))
            })
          }
          
          skippedCount++
          continue
        }
        
        // Log any recommendations even for valid statements
        if (validation.recommendations && validation.recommendations.length > 0) {
          this.log(chalk.yellow(`${progress} ${chalk.yellow('Recommendations:')} ${validation.recommendations.join(', ')}`))
        }
        
        this.log(`${progress} ${chalk.green('✅ Valid:')} ${chalk.white(truncatedSql)}${suffix}`)
        validStatements.push(statement)
        
      } catch (validationError) {
        this.log(chalk.red(`${progress} ❌ Validation failed for statement:`))
        this.log(chalk.white(`  Statement: ${truncatedSql}${suffix}`))
        this.log(chalk.red(`  Error: ${validationError.message}`))
        skippedCount++
      }
    }

    // Add only valid statements to history
    for (const statement of validStatements) {
      history.executed.push({
        id: statement.id,
        hash: statement.hash,
        sql: statement.sql,
        executedAt: new Date().toISOString(),
        recordedOnly: true // Flag to indicate this was recorded without execution
      })
    }

    // Save updated history
    await fs.writeJson(historyFilePath, history, { spaces: 2 })
    
    this.log(chalk.green(`\nSuccessfully recorded ${validStatements.length} valid statement(s) to history`))
    if (skippedCount > 0) {
      this.log(chalk.yellow(`Skipped ${skippedCount} invalid statement(s)`))
    }
    this.log(chalk.gray(`History updated: ${historyFilePath}`))
    this.log(chalk.yellow('Note: SQL statements were NOT executed against the database.'))
  }

  async clearHistory(historyFilePath, environment, sqlFileName) {
    if (await fs.pathExists(historyFilePath)) {
      // Ask for confirmation before clearing history
      const confirmed = await this.confirmClearHistory(environment, sqlFileName, historyFilePath)
      
      if (!confirmed) {
        this.log(chalk.yellow('Clear history operation cancelled.'))
        return
      }
      
      await fs.remove(historyFilePath)
      this.log(chalk.green(`Successfully deleted history file for environment '${environment}' and file '${sqlFileName}'`))
      this.log(chalk.gray(`Deleted: ${historyFilePath}`))
    } else {
      this.log(chalk.yellow(`No history file found for environment '${environment}' and file '${sqlFileName}'`))
      this.log(chalk.gray(`Expected location: ${historyFilePath}`))
    }
    this.log(chalk.yellow('Note: No SQL statements were executed against the database.'))
  }

  async confirmClearHistory(environment, sqlFileName, historyFilePath) {
    // Load and display history information
    const history = await this.loadHistory(historyFilePath)
    const executedCount = history.executed ? history.executed.length : 0
    
    this.log(chalk.cyan(`\n⚠️  You are about to clear ALL history for:`))
    this.log(chalk.white(`   Environment: ${chalk.bold(environment)}`))
    this.log(chalk.white(`   SQL File: ${chalk.bold(sqlFileName)}`))
    this.log(chalk.white(`   History File: ${chalk.gray(historyFilePath)}`))
    this.log(chalk.white(`   Executed Statements: ${chalk.bold(executedCount)}`))
    
    if (executedCount > 0) {
      this.log(chalk.red(`\n   This will permanently delete the record of ${executedCount} previously executed SQL statement(s).`))
      this.log(chalk.red(`   These statements may be re-executed on the next run if they still exist in the SQL file.`))
    }

    // Use readline for confirmation
    const rl = readline.createInterface({
      input: process.stdin,
      output: process.stdout
    })

    return new Promise((resolve) => {
      rl.question(chalk.yellow('\nAre you sure you want to clear ALL history? (y/N): '), (answer) => {
        rl.close()
        resolve(answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes')
      })
    })
  }

  async clearAllHistory(sqldriftDir, environment) {
    // Find all history files for the specified environment
    const files = await fs.readdir(sqldriftDir)
    const historyFiles = files.filter(file =>
      file.startsWith(`${environment}-`) && file.endsWith('-history.json')
    )

    if (historyFiles.length === 0) {
      this.log(chalk.yellow(`No history files found for environment '${environment}'`))
      this.log(chalk.gray(`Searched in: ${sqldriftDir}`))
      this.log(chalk.yellow('Note: No SQL statements were executed against the database.'))
      return
    }

    // Load and analyze all history files
    let totalStatements = 0
    const fileDetails = []
    
    for (const file of historyFiles) {
      const filePath = path.join(sqldriftDir, file)
      const history = await this.loadHistory(filePath)
      const executedCount = history.executed ? history.executed.length : 0
      totalStatements += executedCount
      
      // Extract SQL filename from history filename
      // Format: environment-sqlfilename-hash-history.json
      const withoutEnv = file.replace(`${environment}-`, '')
      const withoutExt = withoutEnv.replace('-history.json', '')
      const parts = withoutExt.split('-')
      // The last part is the hash, everything before that is the SQL filename
      // Filter out empty parts to handle cases like "test1.sql--hash"
      const sqlFileName = parts.slice(0, -1).filter(part => part !== '').join('-')
      
      fileDetails.push({
        file,
        filePath,
        sqlFileName,
        executedCount
      })
    }

    // Ask for confirmation before clearing all history
    const confirmed = await this.confirmClearAllHistory(environment, fileDetails, totalStatements, sqldriftDir)
    
    if (!confirmed) {
      this.log(chalk.yellow('Clear all history operation cancelled.'))
      return
    }

    // Delete all history files
    let deletedCount = 0
    for (const detail of fileDetails) {
      try {
        await fs.remove(detail.filePath)
        this.log(chalk.green(`✅ Deleted history for '${detail.sqlFileName}' (${detail.executedCount} statements)`))
        deletedCount++
      } catch (error) {
        this.log(chalk.red(`❌ Failed to delete ${detail.file}: ${error.message}`))
      }
    }

    this.log(chalk.green(`\nSuccessfully deleted ${deletedCount} history file(s) for environment '${environment}'`))
    this.log(chalk.gray(`Total statements cleared: ${totalStatements}`))
    this.log(chalk.yellow('Note: No SQL statements were executed against the database.'))
  }

  async confirmClearAllHistory(environment, fileDetails, totalStatements, sqldriftDir) {
    this.log(chalk.cyan(`\n⚠️  You are about to clear ALL history for environment '${chalk.bold(environment)}':`))
    this.log(chalk.white(`   Directory: ${chalk.gray(sqldriftDir)}`))
    this.log(chalk.white(`   History Files: ${chalk.bold(fileDetails.length)}`))
    this.log(chalk.white(`   Total Executed Statements: ${chalk.bold(totalStatements)}`))
    
    if (fileDetails.length > 0) {
      this.log(chalk.cyan('\n   Files to be deleted:'))
      fileDetails.forEach((detail, index) => {
        this.log(chalk.white(`   ${index + 1}. ${chalk.bold(detail.sqlFileName)} (${detail.executedCount} statements)`))
      })
    }
    
    if (totalStatements > 0) {
      this.log(chalk.red(`\n   This will permanently delete the record of ${totalStatements} previously executed SQL statement(s).`))
      this.log(chalk.red(`   These statements may be re-executed on the next run if they still exist in their respective SQL files.`))
    }

    // Use readline for confirmation
    const rl = readline.createInterface({
      input: process.stdin,
      output: process.stdout
    })

    return new Promise((resolve) => {
      rl.question(chalk.yellow('\nAre you sure you want to clear ALL history for this environment? (y/N): '), (answer) => {
        rl.close()
        resolve(answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes')
      })
    })
  }

  async viewHistory(historyFilePath, environment, sqlFileName) {
    if (!await fs.pathExists(historyFilePath)) {
      this.log(chalk.yellow(`No history file found for environment '${environment}' and file '${sqlFileName}'`))
      this.log(chalk.gray(`Expected location: ${historyFilePath}`))
      return
    }

    const history = await this.loadHistory(historyFilePath)
    
    if (!history.executed || history.executed.length === 0) {
      this.log(chalk.yellow(`No execution history found for environment '${environment}' and file '${sqlFileName}'`))
      return
    }

    console.log(chalk.cyan(`\n📋 Execution History for ${chalk.bold(sqlFileName)} (Environment: ${chalk.bold(environment)})`))
    console.log(chalk.gray(`History file: ${historyFilePath}`))
    console.log(chalk.gray(`Total records: ${history.executed.length}\n`))

    await this.displayPaginatedHistory(history.executed)
  }

  async displayPaginatedHistory(records) {
    // Sort records by executedAt in descending order (most recent first)
    const sortedRecords = records.sort((a, b) => {
      return new Date(b.executedAt) - new Date(a.executedAt)
    })
    
    const pageSize = 10
    let currentPage = 0
    const totalPages = Math.ceil(sortedRecords.length / pageSize)

    const displayPage = (page) => {
      const start = page * pageSize
      const end = Math.min(start + pageSize, sortedRecords.length)
      const pageRecords = sortedRecords.slice(start, end)

      // Clear screen
      console.clear()
      
      // Display header
      console.log(chalk.cyan('📋 SQL Execution History\n'))
      
      // Display table header
      console.log(chalk.bold(this.formatTableRow('ID', 'Hash', 'SQL', 'Executed At', 'Record Only')))
      console.log(chalk.gray('─'.repeat(110)))

      // Display records
      pageRecords.forEach(record => {
        const truncatedSql = this.truncateText(record.sql.replace(/\s+/g, ' ').trim(), 58)
        const executedAt = new Date(record.executedAt).toLocaleString()
        const recordedOnly = record.recordedOnly ? 'Yes' : 'No'
        const hashShort = record.hash.substring(0, 10) + '...'
        
        console.log(this.formatTableRow(
          record.id.toString(),
          hashShort,
          truncatedSql,
          executedAt,
          recordedOnly
        ))
      })

      // Display pagination info
      console.log(chalk.gray('─'.repeat(110)))
      console.log(chalk.blue(`Page ${page + 1} of ${totalPages} | Records ${start + 1}-${end} of ${sortedRecords.length}`))
      console.log(chalk.gray('\nNavigation: [n]ext, [p]revious, [q]uit, [f]irst, [l]ast'))
    }

    const handleInput = () => {
      return new Promise((resolve) => {
        const rl = readline.createInterface({
          input: process.stdin,
          output: process.stdout
        })

        rl.question(chalk.yellow('Enter command: '), (answer) => {
          rl.close()
          resolve(answer.toLowerCase().trim())
        })
      })
    }

    // Display first page
    displayPage(currentPage)

    // Handle navigation
    while (true) {
      const command = await handleInput()

      switch (command) {
        case 'n':
        case 'next':
          if (currentPage < totalPages - 1) {
            currentPage++
            displayPage(currentPage)
          } else {
            console.log(chalk.yellow('Already on last page'))
          }
          break

        case 'p':
        case 'prev':
        case 'previous':
          if (currentPage > 0) {
            currentPage--
            displayPage(currentPage)
          } else {
            console.log(chalk.yellow('Already on first page'))
          }
          break

        case 'f':
        case 'first':
          currentPage = 0
          displayPage(currentPage)
          break

        case 'l':
        case 'last':
          currentPage = totalPages - 1
          displayPage(currentPage)
          break

        case 'q':
        case 'quit':
        case 'exit':
          console.log(chalk.green('\nExiting history view...'))
          return

        default:
          console.log(chalk.red('Invalid command. Use: [n]ext, [p]revious, [q]uit, [f]irst, [l]ast'))
      }
    }
  }

  truncateText(text, maxLength) {
    if (text.length <= maxLength) {
      return text.padEnd(maxLength)
    }
    return text.substring(0, maxLength - 3) + '...'
  }

  formatTableRow(id, hash, sql, executedAt, recordOnly) {
    const idCol = id.toString().padEnd(4)
    const hashCol = hash.padEnd(12)
    const sqlCol = sql.padEnd(60)
    const executedCol = executedAt.padEnd(20)
    const recordCol = recordOnly.padEnd(12)
    
    return `${idCol} ${hashCol} ${sqlCol} ${executedCol} ${recordCol}`
  }
}

module.exports = SqlDrift